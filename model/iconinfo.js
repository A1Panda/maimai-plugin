...
class IconInfo {
    ...
    async getIconInfo(iconId) {
        try {
            ...
            const response = await adapter.getIconInfo(iconId)
            
            // 获取头像资源
            const iconAsset = await adapter.getIconAsset(parseInt(iconId))
            ...
            const renderData = {
                ...
                // 头像图片转base64
                icon: iconAsset ? `data:image/png;base64,${fs.readFileSync(iconAsset).toString('base64')}` : null,
                // 获取条件
            ...
            logger.error(`[maimai-plugin] 获取头像信息失败: ${error}`)
            return {
                isImage: false,
                message: '获取头像信息失败'
            }
    ...
    // 渲染图片
    async render(data) {
        const imagePath = path.join(process.cwd(), 'temp', 'maimai-plugin', `iconinfo_${Date.now()}.png`)
        ...
        try {
            ...
            // 读取HTML模板
            let template = fs.readFileSync('./plugins/maimai-plugin/resources/html/iconinfo.html', 'utf8')
...
export const iconInfo = new IconInfo()
